...
$_fh4_data = (is_string($tmp=(isset($this->scope["RIGHTNAV"]) ? $this->scope["RIGHTNAV"] : null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);if ($this->isTraversable($_fh4_data) == true){foreach ($_fh4_data as $this->scope['item']){/* -- foreach start output */?><li class="<?php echo (is_string($tmp=$this->scope["item"]["path"]) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
