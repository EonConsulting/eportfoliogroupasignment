...
<?php 
$_fh0_data = (is_string($tmp=(isset($this->scope["JAVASCRIPT"]) ? $this->scope["JAVASCRIPT"] : null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
if ($this->isTraversable($_fh0_data) == true)
{
	foreach ($_fh0_data as $this->scope['script'])
	{
...
<?php 
$_fh1_data = (is_string($tmp=(isset($this->scope["HEADERS"]) ? $this->scope["HEADERS"] : null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
if ($this->isTraversable($_fh1_data) == true)
{
	foreach ($_fh1_data as $this->scope['header'])
	{
/* -- foreach start output */
?>
...
<?php 
$_fh2_data = (is_string($tmp=(isset($this->scope["STYLESHEETLIST"]) ? $this->scope["STYLESHEETLIST"] : null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
if ($this->isTraversable($_fh2_data) == true)
{
	foreach ($_fh2_data as $this->scope['cssurl'])
	{
/* -- foreach start output */
?>
