...
?>nav<?php 
            ...
            <?php $_fh8_data = (is_string($tmp=(isset($this->scope["MAINNAV"]) ? $this->scope["MAINNAV"] : null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);$this->globals["foreach"]['menu'] = array("index"		=> 0,);$_fh8_glob =& $this->globals["foreach"]['menu'];if ($this->isTraversable($_fh8_data) == true){foreach ($_fh8_data as $this->scope['item']){/* -- foreach start output */?><li class="<?php if ((isset($this->scope["item"]["path"]) ? $this->scope["item"]["path"]:null)) {
...
?>
        ...
        <div id="sub-nav" class="navbar navbar-default minor-nav hidden-xs">
            <div class="container">
                ...
                <?php $_fh9_data = (is_string($tmp=(isset($this->scope["SELECTEDSUBNAV"]) ? $this->scope["SELECTEDSUBNAV"] : null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);if ($this->isTraversable($_fh9_data) == true){foreach ($_fh9_data as $this->scope['item']){/* -- foreach start output */?><li<?php if ((isset($this->scope["item"]["selected"]) ? $this->scope["item"]["selected"]:null)) {?> class="active"<?php }?>><a href="<?php echo (is_string($tmp=$this->scope["WWWROOT"]) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
