...
?>
        ...
        <?php 
$_fh1_data = (is_string($tmp=(isset($this->scope["sbdata"]["groups"]) ? $this->scope["sbdata"]["groups"]:null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
if ($this->isTraversable($_fh1_data) == true)
{
	foreach ($_fh1_data as $this->scope['group'])
	{
...
?>
            <div id="views" class="list-group-item">
                ...
                <ul class="list-nested list-group-item-text list-unstyled list-group-item-link">
        <?php 
$_fh2_data = (is_string($tmp=(isset($this->scope["sbdata"]["views"]) ? $this->scope["sbdata"]["views"]:null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
...
?>
            <div class="artefacts list-group-item">
                ...
                <ul class="list-nested list-group-item-text list-unstyled list-group-item-link">
        <?php 
$_fh3_data = (is_string($tmp=(isset($this->scope["sbdata"]["artefacts"]) ? $this->scope["sbdata"]["artefacts"]:null)) ? htmlspecialchars($tmp, ENT_QUOTES, $this->charset) : $tmp);
if ($this->isTraversable($_fh3_data) == true)
{
	foreach ($_fh3_data as $this->scope['artefact'])
	{
/* -- foreach start output */
?>
